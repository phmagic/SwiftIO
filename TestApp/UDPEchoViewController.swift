...
class UDPEchoViewController: NSViewController {
    ...
    func createServer() -> UDPChannel {
        ...
        let udpServer = UDPChannel(label: "example.udp.server", address: serverAddress) {
            (datagram) in
            log?.debug("UDPEcho: Server received - \(datagram)")
            self.udpServer.send(datagram.data, address: datagram.from) {
                result in
            }
        }
    ...
    func createClient() -> UDPChannel {
        ...
        let udpClient = UDPChannel(label: "example.udp.client", address: address) {
            (datagram) in
